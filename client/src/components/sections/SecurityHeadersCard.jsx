...
export const SecurityHeadersCard = ({ securityData }) => {
  const [filter, setFilter] = useState('all');

  if (!securityData) return null;

  const { summary = {}, findings = [] } = securityData;
  ...
  return (
    <Card>
      <CardHeader
        title="Passive Security Headers & Transport"
        subtitle="HTTPS transport encryption, HSTS, CSP, X-Frame-Options, and non-sensitive cookie flags"
        action={
          ...
          {filteredFindings.length === 0 ? (
            ...
            filteredFindings.map((finding) => (
              ...
              >
                ...
                {/* Display Value safely (No raw cookie values exposed) */}
