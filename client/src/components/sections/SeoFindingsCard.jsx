...
export const SeoFindingsCard = ({ seoData }) => {
  const [filter, setFilter] = useState('all');

  if (!seoData) return null;

  const { summary = {}, findings = [] } = seoData;
  ...
  return (
    <Card>
      <CardHeader
        title="SEO Analysis"
        subtitle="Search engine optimization, document title, canonical link, and Open Graph metadata"
        action={
        ...
        <div className="space-y-4">
          {filteredFindings.length === 0 ? (
            <p className="text-sm text-muted italic py-4 text-center">
              No SEO findings match the selected filter.
            </p>
          ) : (
            filteredFindings.map((finding) => (
              ...
              >
                ...
                {/* Display Value if scalar string or object */}
