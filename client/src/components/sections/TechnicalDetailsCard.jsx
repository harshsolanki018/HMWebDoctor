...
export const TechnicalDetailsCard = ({ technicalData }) => {
  const [filter, setFilter] = useState('all');

  if (!technicalData) return null;

  const { summary = {}, findings = [] } = technicalData;
  ...
  return (
    <Card>
      <CardHeader
        title="Technical Response Details"
        subtitle="HTTP response metrics, compression, caching directives, charset encoding, and document payload sizes"
        action={
        ...
        <div className="space-y-4">
          {filteredFindings.length === 0 ? (
            <p className="text-sm text-muted italic py-4 text-center">
              No technical findings match the selected filter.
            </p>
          ) : (
...
TechnicalDetailsCard.propTypes = {
  technicalData: PropTypes.shape({
