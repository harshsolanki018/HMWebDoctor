...
export const PerformanceCard = ({ performanceData }) => {
  const [filter, setFilter] = useState('all');

  if (!performanceData) return null;

  const { summary = {}, findings = [] } = performanceData;
  ...
  return (
    <Card>
      <CardHeader
        title="Passive Resource & Performance Optimization"
        subtitle="Script execution attributes, stylesheet footprints, resource hints, DOM element complexity, and image markup"
        action={
        ...
        <div className="space-y-4">
          {filteredFindings.length === 0 ? (
            <p className="text-sm text-muted italic py-4 text-center">
              No performance findings match the selected filter.
            </p>
          ) : (
