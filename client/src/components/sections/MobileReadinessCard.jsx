...
export const MobileReadinessCard = ({ mobileData }) => {
  const [filter, setFilter] = useState('all');

  if (!mobileData) return null;

  const { summary = {}, findings = [] } = mobileData;
  ...
  return (
    <Card>
      <CardHeader
        title="Passive Mobile Responsiveness Markup Analysis"
        subtitle="Viewport zoomability, touch keyboard input types, virtual keyboard inputmode hints, autofill tokens, and mobile presentation meta tags"
        action={
        ...
        <div className="space-y-4">
          {filteredFindings.length === 0 ? (
            <p className="text-sm text-muted italic py-4 text-center">
              No mobile findings match the selected filter.
            </p>
          ) : (
